...
类： 定义类时，类名的命名规范-->大驼峰命名法 如 UserInfo、 UserAccount
 ...
 1. __init__()  初始化方法 每次创建对象的时候，会自动触发该类的__int__函数
 2. __str__()  字符串表示的方法 当用print()函数打印对象的时候，会自动调用该对象(所在类)的str魔法方法。
            该魔法方法默认打印的是对象的地址值，无意义，一般都会重写，改为打印 队形的各个属性值
...
5. __def__(): 当.py文件执行结束，或者手动del释放对象资源，会自动调用该函数。
属性：
